...
public class Advent03
{
    ...
    void run()
    {
        ...
        foreach (var character in input)
        {
            ...
            {
                list_special_characters.Add(character);
            }
        }

        list_special_characters = list_special_characters.Distinct().ToList(); //removes Duplicates
        //list_special_characters.ForEach(Console.WriteLine);

        string[] lines = input.Split(new[] { "\n" }, StringSplitOptions.RemoveEmptyEntries); //separating each line
        

        for (int a = 0; a < lines.Length; a++)
        {
            List<int> integers_in_line = FindMultiDigitNumbers(lines[a]);
            List< int> index_of_special_character_above = new List< int>();
            List< int> index_of_special_character_below = new List< int>();
            int counter_for_character = 0; //counter to avoid duplicates
            try
            {
                foreach (var character in lines[a-1])
                {
                    if (list_special_characters.Contains(character))
                    {
                        index_of_special_character_above.Add(lines[a-1].IndexOf(character, counter_for_character));
                    }

                    counter_for_character++;
                }
            }
            catch (Exception e) {/*ingored*/}
            ...
            try
            {
                foreach (var character in lines[a+1])
                {
                    if (list_special_characters.Contains(character))
                    {
                        index_of_special_character_below.Add(lines[a+1].IndexOf(character, counter_for_character));
                    }
            ...
            foreach (var number in integers_in_line)
            {
                int index_of_number = lines[a].IndexOf(number.ToString(), counter_for_index);
                int length_of_number = number.ToString().Length;
                counter_for_index = index_of_number;
                //Console.WriteLine($"Number: {number}, Index: {index_of_number}, Länge: {length_of_number}");
                try //check for special character left
                {
                    if (list_special_characters.Contains(lines[a][index_of_number - 1]))
                    {
                        result.Add(number);
                    }
                }
                ...
                try //check for special character right
                {
                    if (list_special_characters.Contains(lines[a][index_of_number + length_of_number]))
                    {
                        result.Add(number);
                    }
                }
                ...
                try //check for special character above
                {
                    foreach (var index in index_of_special_character_above)
                    {
                        if (index >= index_of_number - 1 && index <= index_of_number + length_of_number)
                        {
                            result.Add(number);
                        }
                    }
                ...
                try //check for special character below
                {
                    foreach (var index in index_of_special_character_below)
                    {
                        if (index >= index_of_number - 1 && index <= index_of_number + length_of_number)
                        {
                            result.Add(number);
        ...
        List<double> test = FindMultiDigit(lines[2]);
        //test.ForEach(Console.WriteLine);
    }

    List<int> FindMultiDigitNumbers(string inputString) //generated by chatgpt
    {
        List<int> result = new List<int>();

        // Definiere den regulären Ausdruck für mehrstellige Zahlen
        Regex regex = new Regex(@"\b\d+\b");

        // Finde Übereinstimmungen im Eingabe-String
        MatchCollection matches = regex.Matches(inputString);

        // Verarbeite die Übereinstimmungen und füge sie zur Ergebnisliste hinzu
    ...
    List<double> FindMultiDigit(string inputString) //generated by chatgpt
    {
